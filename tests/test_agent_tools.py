...
@pytest.fixture()
def ws(tmp_path):  # type: ignore[no-untyped-def]
    return tmp_path


class TestReadFile:
    def test_reads_existing_file(self, ws):  # type: ignore[no-untyped-def]
        (ws / "hello.txt").write_text("hello world")
        assert read_file("hello.txt", ws) == "hello world"

    def test_missing_file_returns_error(self, ws):  # type: ignore[no-untyped-def]
        result = read_file("nope.txt", ws)
        assert result.startswith("Error:")

    def test_path_traversal_rejected(self, ws):  # type: ignore[no-untyped-def]
        result = read_file("../../etc/passwd", ws)
        assert result.startswith("Error:")


class TestWriteFile:
    def test_creates_file(self, ws):  # type: ignore[no-untyped-def]
        result = write_file("out.txt", "content", ws)
        assert "Written" in result
        assert (ws / "out.txt").read_text() == "content"

    def test_creates_parent_dirs(self, ws):  # type: ignore[no-untyped-def]
...
class TestListDirectory:
    def test_lists_entries(self, ws):  # type: ignore[no-untyped-def]
        (ws / "a.py").write_text("")
        (ws / "b.py").write_text("")
    ...
    def test_non_directory_returns_error(self, ws):  # type: ignore[no-untyped-def]
...
class TestSearchCodebase:
    def test_finds_match(self, ws):  # type: ignore[no-untyped-def]
        (ws / "main.py").write_text("def hello():\n    pass\n")
        result = search_codebase("hello", ".", ws)
        assert "main.py" in result
        assert "hello" in result

    def test_no_match_message(self, ws):  # type: ignore[no-untyped-def]
